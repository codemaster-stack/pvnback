...
exports.register = async (req, res) => {
  try {
    ...
    // 🔹 FIXED: Auto-create account for new user
...
// @desc    Register Admin (admins don't need accounts)
exports.registerAdmin = async (req, res) => {
  try {
    const { fullName, email, password } = req.body;
...
exports.login = async (req, res) => {
  try {
    ...
    const user = await User.findOne({ email }).select("+password");
    if (user && (await user.matchPassword(password))) {
      // 🔹 FIXED: Also return account info on login if user has account
      let accountInfo = {};
      if (user.role === "user") {
      ...
      res.json({
        ...
        phone: user.phone,
        role: user.role,
        ...accountInfo, // Spread account info
        token: generateToken(user._id),
      });
    } else {
      res.status(401).json({ message: "Invalid email or password" });
...
// ... rest of your existing code (forgotPassword, resetPassword, etc.)
exports.forgotPassword = async (req, res) => {
  try {
    const { email } = req.body;
